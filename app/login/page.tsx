...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (result?.ok) window.location.href = callbackUrl || "/";
    } catch {
      ...
      <div className="max-w-md w-full bg-white rounded-lg shadow-xl p-8 mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <p className="text-center mt-4 text-sm text-gray-600">
            First time? <Link href="/signup" className="text-blue-600 hover:text-blue-800 font-medium">Create account</Link>
          </p>
        </form>
      </div>
    </div>
